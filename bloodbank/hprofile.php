				...
				<div class="card">
					...
					<div class="card-body">
					   <form action="file/updateprofile.php" method="post">
						...
						<input type="text" name="hname" value="<?php echo $row['hname']; ?>" class="form-control mb-3" required>
						<label class="text-muted font-weight-bold">Hospital Email</label>
						<input type="email" name="hemail" value="<?php echo $row['hemail']; ?>" class="form-control mb-3" required>
						<label class="text-muted font-weight-bold">Hospital Password</label>
						<input type="text" name="hpassword" value="<?php echo $row['hpassword']; ?>" class="form-control mb-3" required minlength="6" maxlength="8">
						<label class="text-muted font-weight-bold">Hospital Phone Number</label>
						<input type="tel" name="hphone" value="<?php echo $row['hphone']; ?>" class="form-control mb-3" required pattern="[0,6-9]{1}[0-9]{9,11}" minlength="10" maxlength="10" >
						<label class="text-muted font-weight-bold">Hospital City</label>
						<input type="text" name="hcity" value="<?php echo $row['hcity']; ?>" class="form-control mb-3" required>
