...
if (!isset($_SESSION['hid'])) {
	...
	$hid = $_SESSION['hid'];
	$sql = "select donorrequest.*, donors.* from donorrequest, donors where hid='$hid' && donorrequest.did=donors.id";
	...
	<body>
		...
		<div class="container cont">
			...
			<table class="table table-responsive table-striped rounded mb-5">
				<tr>
					<th colspan="9" class="title">Blood requests</th>
				</tr>
				<tr>
					<th>#</th>
					<th>Name</th>
				...
				<?php while ($row = mysqli_fetch_array($result)) { ?>
					...
					<tr>
						...
						<td><?php if ($row['status'] == 'Accepted') { ?> <a href="" class="btn btn-success disabled">Accepted</a> <?php } else { ?>
								<a href="file/accept.php?reqid=<?php echo $row['reqid']; ?>" class="btn btn-success">Accept</a>
							<?php } ?>
						</td>
						<td><?php if ($row['status'] == 'Rejected') { ?> <a href="" class="btn btn-danger disabled">Rejected</a> <?php } else { ?>
